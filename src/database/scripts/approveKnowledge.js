...
//
//   npx tsx src/database/scripts/approveKnowledge.js                 # show status + list pending
...
import { approveKnowledgeChunks, getKnowledgeStatusCounts } from '../../server/db/knowledge.queries.js';
...
async function main() {
  ...
  if (!all && !kind && !topic) {
    ...
    if (pending.length) {
      ...
      console.log('\nApprove with: --all | --kind <kind> | --topic <topic>');
    } else {
      console.log('\nNothing pending.');
    }
    return;
  ...
  const n = await approveKnowledgeChunks({
